...
const schema = z.object({
  horseName: z.string().min(1).max(100),
  invitedEmail: z.string().email(),
  role: z.enum(["DEMI_PENSION", "COACH"]),
});

const ROLE_LABEL: Record<"DEMI_PENSION" | "COACH", string> = {
...
export async function POST(req: NextRequest) {
  ...
  const { horseName, invitedEmail, role } = parsed.data;
  ...
  const subject = `${inviterName} vous invite à suivre ${horseName} sur Horsetrack`;
  const bodyText = [
    `${inviterName} vous invite à accéder au suivi de ${horseName} sur Horsetrack, ${ROLE_LABEL[role]}.`,
    "",
