...
export type JournalEntry = {
  ...
  /** Photo principale optionnelle ("photo du jour") : URI locale (copiée dans
   * le stockage persistant de l'app via lib/imagePicker.ts, comme les photos
   * de cheval/document) tant qu'elle n'a pas encore été synchronisée, puis URL
   * signée Supabase une fois restaurée depuis le cloud (cf. lib/cloudSync.ts
   * pullJournalEntries). Null tant qu'aucune photo n'a été ajoutée. Une seule
   * photo par entrée pour V1, pas de galerie. */
  photoUri: string | null;
  /** Chemin dans le bucket Storage "horse-photos" (sous-dossier "journal/")
   * une fois synchronisé — permet de ne pas re-uploader la même photo à
   * chaque synchro. Géré exclusivement par la synchro, jamais modifié
   * directement par le formulaire. Null tant que pas encore synchronisé (ou
   * pas de photo). */
...
type AgendaContextValue = {
  ...
  addJournalEntry: (
    entry: Omit<JournalEntry, "id" | "horseId" | "photoPath"> & { horseId?: string | null }
  ) => void;
...
export function AgendaProvider({ children }: { children: ReactNode }) {
  ...
  const addJournalEntry = useCallback(
    ...
    (entry: Omit<JournalEntry, "id" | "horseId" | "photoPath"> & { horseId?: string | null }) => {
      ...
      const next: JournalEntry = { ...rest, id, horseId, photoPath: null };
      setJournal((list) => [...list, next]);
      // Best-effort, jamais bloquant : cf. lib/cloudSync.ts. Le photoPath
      // résultant est reporté localement pour ne pas re-uploader la même
      // photo à la prochaine synchro (cf. pushJournalEntry), même logique que
      // addDocument.
